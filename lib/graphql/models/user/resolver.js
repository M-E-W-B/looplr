...
const resolvers = {
  Query: {
    users: async function(root, { pagination, orderings, filters }, ctx) {
    ...
    followers: async function(
      root,
      { user_id, pagination, orderings, filters },
      ctx
    ) {
      const { userRepository } = ctx;

      try {
        const edges = await userRepository.getFollowers(
          user_id,
          pagination,
          orderings,
          filters
    ...
    followings: async function(
      ...
      try {
        const edges = await userRepository.getFollowings(
          user_id,
          pagination,
          orderings,
          filters
