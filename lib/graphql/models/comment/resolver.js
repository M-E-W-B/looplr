...
const resolvers = {
  Query: {
    ...
    getCommentsByUserId: async function(root, { user_id }, ctx) {
      const { commentRepository } = ctx;
      try {
        const comments = await commentRepository.getCommentsByUserId(user_id);
        return comments;
      } catch (err) {
        throw new ListFetchError({
    ...
    getCommentsOn: async function(root, { entity_id }, ctx) {
      const { commentRepository } = ctx;
      try {
        const comments = await commentRepository.getCommentsOn(entity_id);
        return comments;
      } catch (err) {
        throw new ListFetchError({
          message: "Unable to fetch the comments.",
    ...
    getInactiveSkus: async function(root, args, ctx) {
      const { commentRepository } = ctx;
      try {
